...
for r in regions:
    ...
    for cand in r:
        ...
        nb = []
        for l in lb(cand): border.add(l)
        fence += len(nb)
        for b in bor(cand): border.add(b)

    ans1 += fence * len(r)
    for b in border:
        if int(b[1] * 10) % 10 == 5:
            if b[1] in verbor: verbor[b[1]].append(b[0])
            else: verbor[b[1]] = [b[0]]
        else:
            if b[0] in horbor: horbor[b[0]].append(b[1])
            else: horbor[b[0]] = [b[1]]
    ...
    for h in horbor:
        ...
        for i in range(0, len(horbor[h]) - 1):
            if horbor[h][i] != horbor[h][i + 1] - 1: count += 1
            else:
                if (horbor[h][i] + horbor[h][i + 1]) / 2 in verbor: 
                    if h-0.5 in verbor[(horbor[h][i] + horbor[h][i + 1]) / 2]:
                        if h+0.5 in verbor[(horbor[h][i] + horbor[h][i + 1]) / 2]: count += 2
    for v in verbor:
        verbor[v].sort()
        count += 1
        for i in range(0, len(verbor[v]) - 1):
            if verbor[v][i] != verbor[v][i + 1] - 1: count += 1
    ans2 += count * len(r) 
